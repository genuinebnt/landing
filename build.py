...
.mglow {
  ...
  /* Wide and short, not a circle. A perfect radial disc reads as a torch
     pointed at the page; three offset lobes at different sizes read as light
     falling across a surface, which is what the grid wants behind it. */
  width: 1180px; height: 660px; margin: -330px 0 0 -590px;
  pointer-events: none;
  background:
    radial-gradient(58% 62% at 36% 44%, rgba(var(--mglow-rgb), .17), transparent 70%),
    radial-gradient(46% 52% at 66% 60%, rgba(var(--mglow-rgb), .11), transparent 72%),
    radial-gradient(52% 38% at 56% 26%, rgba(var(--mglow-rgb), .08), transparent 76%);
  filter: blur(26px);
...
/* The grid lines themselves brighten where the light is. Same 72px lattice as
   .gridpan, drawn stronger and masked to a soft disc that rides the pointer, so
   the grid appears lit rather than a second grid appearing on top of it. */
.gridlit {
  position: absolute; top: 0; left: 0;
  width: 620px; height: 620px; margin: -310px 0 0 -310px;
  pointer-events: none;
  background-image:
    linear-gradient(rgba(var(--mglow-rgb), .30) 1px, transparent 1px),
    linear-gradient(90deg, rgba(var(--mglow-rgb), .30) 1px, transparent 1px);
...
(function () {
  ...
  // Inserted before the grid so the grid paints over it.
  grid.parentNode.insertBefore(el, grid);

  // The lit grid goes *after* .gridpan — it is the highlight on top of the
  // lattice, where the glow is the light underneath it.
  var lit = document.createElement('div');
  lit.className = 'gridlit%(rekey)s';
  lit.setAttribute('aria-hidden', 'true');
  lit.style.setProperty('--mglow-rgb', '%(rgb)s');
  grid.parentNode.insertBefore(lit, grid.nextSibling);
  ...
  var tx = window.innerWidth * 0.56, ty = 300;
  var x = tx, y = ty, raf = 0;
  ...
  el.style.transform = lit.style.transform = 'translate3d(' + x + 'px,' + y + 'px,0)';

  // Card spotlight: one delegated listener, and only the card under the
  // pointer ever gets a custom property written to it.
