...
.mfluid i:nth-child(1) { width: 620px; height: 460px; margin: -230px 0 0 -310px;
  background: radial-gradient(circle, rgba(var(--mglow-rgb), .30), transparent 68%); }
.mfluid i:nth-child(2) { width: 460px; height: 560px; margin: -280px 0 0 -230px;
  background: radial-gradient(circle, rgba(var(--mglow-rgb), .22), transparent 70%); }
...
(function () {
  ...
  // The mass does not travel to the pointer. Each lobe has a home it sits in,
  // and the pointer only pulls on it — hard when it is close, not at all when
  // it is far. So the field stays where it is and *bulges* toward you, which is
  // what a liquid under glass does; chasing the cursor is what a sticker does.
  var cfg = [
    { hx: 0.32, hy: 0.34, pull: 210, reach: 520, ax: 30, ay: 22, fx: 0.00041, fy: 0.00057, ph: 0   },
    { hx: 0.58, hy: 0.26, pull: 150, reach: 620, ax: 44, ay: 32, fx: 0.00033, fy: 0.00047, ph: 2.1 },
    { hx: 0.46, hy: 0.58, pull: 110, reach: 700, ax: 58, ay: 42, fx: 0.00025, fy: 0.00036, ph: 4.2 }
  ];
  var px = null, py = null;                       // pointer, page coords
  var st = cfg.map(function () { return { x: 0, y: 0 }; });

  function frame(now) {
    var vw = innerWidth, vh = innerHeight, top = scrollY;
    for (var i = 0; i < cfg.length; i++) {
      var c = cfg[i], p = st[i];
      // home drifts on its own slow sine, so the field breathes at rest
      var hx = c.hx * vw + Math.sin(now * c.fx + c.ph) * c.ax;
      var hy = top + c.hy * vh + Math.cos(now * c.fy + c.ph) * c.ay;
      ...
      if (px !== null) {
        ...
        if (d > 1 && d < c.reach) {
          // falloff: full lean when the pointer is on top of the lobe, nothing
          // at the edge of its reach — squared so the influence is local
          var f = 1 - d / c.reach; f *= f;
          gx += (dx / d) * c.pull * f;
          gy += (dy / d) * c.pull * f;
        }
      }
      p.x += (gx - p.x) * 0.06;   // viscosity: it eases into the new shape
      p.y += (gy - p.y) * 0.06;
  ...
  addEventListener('pointermove', function (e) {
    px = e.clientX; py = e.clientY + scrollY;
  }, { passive: true });
  addEventListener('pointerleave', function () { px = py = null; }, { passive: true });
